...
/**
Function printErrorCommand:

Input:
    file_name: the name of input file.
    line_number: The line where the error was received
    commandResult_in: the error.
    firstError: true if it is the first error. false otherwise.
*/
void printErrorCommand(char * file_name, int line_number, CommandResult commandResult_in, bool firstError){
    if(firstError){
        printf("error(s) at file '%s.as':\n", file_name);
    }
    ...
    switch(commandResult_in){
        ...
        case COMMAND_STRING_ERROR:
            break; /* These errors do not belong to the user */
        ...
        case COMMAND_OPCODE_UNRECOGNIZED: printf("error: invalid opcode.");
            break;
        case COMMAND_SYMBOL_REDECLARED: printf("error: redeclared label.");
    }
    printf("\n");
}
...
Program programCreate(FILE* file_in, char* file_name ,Memory memoryData_in,
    ...
    Program newProgram = (Program)malloc(sizeof(struct program_t));
    ...
    *result_out = PROGRAM_SUCCESS;
    ...
    if(errors){
        printf("build file '%s' failed: %d error(s).\n",file_name, errors);
        *result_out = PROGRAM_FIRST_PASS_FAIL;
    }
    newProgram->numOfCommands = indexCommands;
    return newProgram;
}


ProgramResult programSecondPass(Program program, Memory memoryData_in, Memory memoryCode_in, Stack stack_in, char* file_name){
    int lengthOfData; /* ID */
    int lengthOfCode; /* CD */
    int i, j; /* index */
    char* label_out;
    ...
    if (errors){
        return PROGRAM_SECOND_PASS_FAIL;
    }
